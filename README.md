...
- **Comparative / differential attribution** (`NUDGE-METHOD-010`) — the SAME perturbation
  ...
  **additive offset on one context's *perturbed* cells** (invisible to the control-keyed depth
  ...
  a **multiplicative scale on one context's *perturbed* cells** (which fakes a `ceiling-diff` /
  `gain-diff` and slips past *both* the depth ratio and the additive guard) is a **measured,
  partly-unguardable** confound — a *small* such scale is degenerate with a genuine small
  ceiling/gain difference (no OFF statistic separates them), so NUDGE **abstains** on gain/ceiling
  differences when the OFF mode is not resolvable, and resolves a ceiling difference only when it is
  large enough to be un-fakeable by a bounded scale — an honest **bound**, sacrificing small genuine
  ceilings rather than risk a confident-wrong. Never a spurious mechanism
