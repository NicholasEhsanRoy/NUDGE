...
| OED (differentiability moat) | ✅ (synthetic) | **Gradient-based Optimal Experimental Design `inference/oed.py` (`NUDGE-METHOD-014`, `NUDGE-LIM-024`) — the white-box advantage a black-box ODE solver can't offer.** Everywhere else NUDGE differentiates a *fit loss* w.r.t. *parameters*; this differentiates an *identifiability criterion* w.r.t. the *experiment*. Because the forward model is differentiable, the Fisher Information `FIM(φ)=J(φ)ᵀJ(φ)/σ²` (`J=∂observe/∂θ` by autodiff through a self-contained RK4 `lax.scan`; touches neither `fit.py` nor `core/`) is a differentiable function of a design parameter φ (the measurement times), so `∂criterion/∂φ` is available by autodiff and we **gradient-ASCEND φ to the exact optimal experiment φ\***. Criteria: D-opt `log det FIM`, A-opt `−tr FIM⁻¹`, E-opt `λ_min`, and a **targeted reciprocal-CRLB** `−log[FIM⁻¹]_ii` for one sloppy parameter. **Makes the gLV α⇄βᵢᵢ directional abstention (`NUDGE-METHOD-012`) EXACT** — "sample the transient to break the tie" becomes a computed schedule. **MEASURED** (logistic growth, target α, m=8 times): naive near-equilibrium design near-singular (cond 136, min-eig 54, CRLB(α)=7.3e-3); φ\* puts samples in the transient → **CRLB(α) 31.5× better, FIM min-eig 17.8× better, cond→9.5**; all three objectives resolve it; gLV community **600×**. White-box vs black-box honestly: on a 1-D knob the gradient's zero-crossing lands on a 200-pt grid's optimum (t1≈3.70), while a structured grid guaranteeing the optimum over m free times costs `rᵐ` evals (m=8→65k/1.7M; m=16→4.3e9/2.8e12) — the moat is the scaling. **Local OED (`NUDGE-LIM-024`):** the optimum + gains are MEASURED at the nominal θ₀, not extrapolated; guarded-ridge inverse (never understates the CRLB); grid baseline shipped to CHECK the gradient. It's a design *recommendation*, not an attribution verdict → can't emit a confident-wrong mechanism call. **Rank-deficient-naive honesty (`NUDGE-LIM-029`):** if the naive baseline does NOT identify the target (FIM smallest eigenvalue at/below its own relative-ridge floor → the guarded CRLB is a Tikhonov artifact, true CRLB unbounded), the tool refuses the false-precise finite factor — `naive_rank_deficient=True`, `crlb_improvement`/`min_eig_improvement` marked LOWER BOUNDS with a note; the trigger is a MEASURED curvature comparison (min-eig vs ridge floor, >12 orders of magnitude of separation) + a threshold-free ridge-doubling test on the target direction, so a merely ill-conditioned-but-informative design (ad_qsp 8-pt default, cond≈6981 → ×259 CRLB) is UNCHANGED. 10 fast + 5 slow tests (`tests/inference/test_oed.py`). `nudge oed --model {logistic,glv} --objective {d_opt,a_opt,e_opt,crlb}` CLI + `service.oed_demo` + Mechanism Card (`NUDGE-METHOD-014`) + `notebooks/Optimal_Experimental_Design.ipynb` + the **unified moat demo `notebooks/4_Differentiable_OED_Moat.ipynb`** (ties the OED gradient to matrix-free scale in one story: naive design near-singular → `∂(D-opt)/∂φ` resolves α 31.3×/31.5× → the same FIM computed matrix-free stays flat where dense OOMs); FINDINGS "Optimal Experimental Design". Real-data lock-in deferred (`needs_data`). Additive/opt-in. |
| AD QSP clinical demo | ✅ (real model, synthetic cohort) | **Flagship real-model clinical demo — matrix-free population identifiability + gradient OED on a REAL, published, open Alzheimer's amyloid-β QSP model (`mechanisms/ad_qsp.py`, `NUDGE-LIM-026`).** Points the matrix-free identifiability (`NUDGE-LIM-023`) + gradient OED (`NUDGE-METHOD-014`) at the **Proctor et al. 2013** AD amyloid model (PLoS ONE 8(9):e73631, PMID 24098635; BioModels `BIOMD0000000488`, **CC0**), calibrated against a **synthetic ground-truth cohort** (no patient data). A deep-research subagent downloaded + machine-parsed the SBML (GO, CC0 confirmed; Geerts 2023 CONDITIONAL-GO deferred); the **full 64-state** network (73 params, 112 reactions) is transcribed faithfully into `mechanisms/_proctor2013.py` by an offline MathML→JAX generator (`scripts/vv/gen_proctor2013.py`). **Action 2 (`scripts/demo_matrix_free_scale.py`):** population-scale calibration (N subjects × 12 subject-specific params → thousands of free params at FIXED integrated state) — MEASURED, dense `jacfwd` **OOM-killed at n_free≥1000** under a 2.5 GB systemd cap while the **matrix-free FIM matvec stays flat ~0.57 GB (1.01×)** and returns the same verdict (honest rank-deficient regime → certified `unidentifiable`); single-subject identifiability is **`sloppy-but-predictive`** (cond ≈7e8, span 8.8 decades) and **names the plaque-growth gain `k_pg` / threshold `K_pg`** + disaggregation `k_dis` as the sloppiest. **Action 3 (`scripts/demo_gradient_oed.py`):** the antibody-binding `k_on` ⇄ microglial-clearance `k_gl` pair is **perfectly confounded** by a naive baseline+end amyloid-PET schedule (corr ≈1.000, cond 8.5e3), and gradient OED (D-optimality) resolves it — MEASURED **×220 CRLB / ×205 smallest-eigenvalue** lift, cond→33 (local OED at θ₀, `NUDGE-LIM-024`) — with the 95%-confidence-ellipse-collapse GIF (`viz/oed`). **Honesty (`NUDGE-LIM-026`, load-bearing):** synthetic cohort (never real patients) + demo-scaled dimensionless constants — the published stiff seconds-to-years parameterization can't be integrated by explicit RK4 (measured: diverges within days), so the demo keeps the published topology + rate-law forms with non-dimensionalized constants; the full published network is retained verbatim so ground truth + demo model are never conflated. 0 confident-wrong (identifiability abstains/`sloppy-but-predictive`; OED is a design recommendation, not an attribution verdict — and on a rank-deficient naive baseline it now reports a LOWER BOUND, not a false-precise finite factor, `NUDGE-LIM-029`). Additive; frozen `fit.py`/`core/` untouched. `tests/mechanisms/test_ad_qsp.py` (8 fast + 2 slow, incl. the `NUDGE-LIM-029` rank-deficient-naive guard + byte-for-byte ×259 preservation); FINDINGS "AD amyloid-β QSP clinical demo". **NLME SCALE-WALL — the coupled (arrowhead) population model (`make_ad_nlme_cohort_predict_fn`, registered `ad_qsp_nlme`, `NUDGE-LIM-028`).** The Action-2 cohort above is INDEPENDENT-subjects → its FIM is BLOCK-DIAGONAL (per-subject 12×12 blocks), so a competent agent decomposes it and never needs a big matrix — a "dense OOMs" story there is a STRAWMAN. The NLME cohort shares population hyperparameters (geometric-mean `μ` + fixed effects `φ` + optional log-spread `ω` via RE-prior pseudo-obs), each subject drawing a random effect `r_i` around them; since `μ`/`φ`/`ω` enter EVERY subject, the joint FIM over `[μ|ω?|φ|r₀…r_{N-1}]` is a genuine ARROWHEAD (MEASURED: `max|FIM[border,subj0]|≈4.3e2` vs `max|FIM[subj0,subj1]|=0.0`) — block-summing no longer applies. **MEASURED (`scripts/demo_nlme_scale.py`):** dense `jacfwd` OOM-killed at n_subjects≥300 (n_free≥912) under a 2.5 GB cap (the tangent fan-out ∝ n_params·n_steps·cohort_state) while matrix-free stays ~0.56→0.89 GB across N=100→2500 (7512 joint params; 1.59× — grows only with cohort state, not the n_obs·n_params product), same `unidentifiable` verdict, 0 label-mismatch. The `NUDGE-LIM-023` rank-deficiency fail-safe is PRESERVED on the coupled model. **Honesty (`NUDGE-LIM-028`):** arrowhead is in-principle Schur-decomposable, so this is the MEASURED dense-OOM-vs-matrix-free-flat contrast (+ a from-scratch NumPy model's lack of autodiff → finite-diff matvecs = O(n_params) forward solves/matvec, intractable), NOT an impossibility claim; synthetic cohort + demo-scaled constants; real AD cohorts (ADNI/A4/DIAN/OASIS-3) exist and population-NLME QSP amyloid fits with an FIM step are published precedent (Ramakrishnan 2023, CPT:PSP) but are all DUA/registration-gated → synthetic is the honest ceiling. WITHOUT-arm: `scripts/demo_ab/ad_qsp_nlme_forward.py` + `cohort_nlme.npz` (NumPy, no `nudge`). `tests/mechanisms/test_ad_qsp.py` (+5 fast +1 slow NLME); FINDINGS "AD QSP NLME (hierarchical)". |
...
(M3; NLL gap 0.005→0.098, ×20) **behind a MEASURED identifiability gate + graceful monostable
degradation** (`NUDGE-LIM-025`, red-team P7): the pure NLL-gap resolution was blind to a
threshold-DOMINATED large-gain perturbation whose perturbed condition slid through the fold
(confident-wrong `threshold` for a true gain), so a resolved mechanism is now trusted only if
the joint (winner, runner-up) Laplace posterior shows the runner-up is NOT identifiably
displaced from no-change (`_resolution_contamination`, cut 0.5 log-units; genuine ≤0.12 vs the
hole ≈1.0) — else abstain; and `lna_reliable`, which **abstains loudly** at low depth / near a
bifurcation / when monostable (M4). Full write-up: `scripts/vv/FINDINGS.md`
"Covariance attribution" + "§P7".
