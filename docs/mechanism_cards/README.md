...
| [constitutive_control](constitutive_control.md) | attribution-method | `NUDGE-METHOD-011` | Constitutive-reporter calibration control: separate CIRCUIT ultrasensitivity from a NONLINEAR READOUT (the `NUDGE-LIM-006` mitigation). A control driving the reporter at KNOWN doses (bypassing the circuit) anchors the readout (readout params ONLY, no leak); a profile over circuit n rejects "no switch" → biological-switch, or abstains — never a bare knob, and never point-identifies n (`NUDGE-LIM-018`) |
| [lotka_volterra_attribution](lotka_volterra_attribution.md) | attribution-method | `NUDGE-METHOD-012` | Temporal / gLV trajectory-fit attribution (a NEW dynamical-systems domain: microbiome ecology): attribute a community perturbation to a change in growth (α) / interaction (β) / antibiotic-susceptibility (ε), or abstain. ε is the identifiable positive; the α⇄βᵢᵢ pair is degenerate near equilibrium (`Kᵢ=−αᵢ/βᵢᵢ`) and abstains with the degeneracy MEASURED by the Laplace curvature (`NUDGE-LIM-020`) |
