...
4c. **unresolved — the perturbed-only multiplicative scale (`NUDGE-LIM-016`, P4/P5 — BOUNDED).**
   A constant *multiplicative* factor `c` on ONE context's *perturbed* cells (control clean)
   slips past gate 4 (`depth_ratio` ≈ 1) and gate 4b (`off_shift` ≈ 1), and BIC assigns it to the
   **ceiling** OR **gain** channel. P5 MEASURED (both regimes, 4 seeds) that a *small* such factor
   is UNGUARDABLE by the OFF statistics — a genuine ceiling/gain difference also moves `off_scale`
   AND `off_shift`, so the confident-wrong confound lands at `off_scale` 1.08–1.28 *inside* the
   genuine range (the P4 "clean gap" held only for large factors). NUDGE therefore: **(i)** abstains
   on gain-diff AND ceiling-diff when either context's OFF mode is not resolvably above zero
   (`off_resolvability` = OFF-cluster level ÷ spread of the clean control below `0.25`; measured RT
   ≤ 0.136 vs TEST ≥ 0.374) — a uniform scale is washed out there; **(ii)** in the resolvable
   regime, resolves a **ceiling-diff** only when its fitted `|log2 v_max ratio| ≥ 0.60` (a large,
   un-fakeable difference — a scale surviving the `off_scale` band fakes only `|log2| ≤ 0.48`); a
   resolvable **gain-diff** is trustworthy; **(iii)** still catches a GROSS scale via the
   `off_scale` band `[0.80, 1.30]` (now on the gain channel too). **BOUNDED, not closed:** the
   honest price is over-abstention on a genuine SMALL ceiling difference (< ~×1.5) and on the whole
   ceiling/gain channel in a near-zero-basal regime; a large genuine ceiling and resolvable gain
   still resolve. Deflation stays degenerate with a genuine ceiling reduction (the P4 bound).
...
| A GROSS multiplicative scale on ONE context's *perturbed* cells (control clean) faking a `*-diff` (P4) | the OFF-cluster-scale band guard (gate 4c-i, `[0.80, 1.30]`, gain + ceiling) → `unresolved` (`test_decoy_multiplicative_perturbed_scale_abstains`) | `NUDGE-LIM-016` |
| A SMALL multiplicative scale (c ~ 1.15–1.25) faking a `gain-diff` / `ceiling-diff` — UNGUARDABLE by OFF statistics (P5) | resolvability gate (near-zero OFF → abstain both channels) + ceiling-magnitude gate (`\|log2\| < 0.60` → abstain) → `unresolved`, both regimes (`test_decoy_small_multiplicative_scale_abstains`); BOUNDED — sacrifices small genuine ceilings | `NUDGE-LIM-016` |
