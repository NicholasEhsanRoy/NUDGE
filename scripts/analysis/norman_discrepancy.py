...
from nudge.inference.bridge import _condition_mask, _norm_counts, combo_effect_scores
...
def axis_diag(adata, ga, gb, n_top=2000):
    ...
    return dict(norm_vA=float(na), norm_vB=float(nb), cos_AB=cos,
                ...
                resid_norm=resid_norm, resid_on_axis=resid_par,
                resid_off_axis=resid_perp)
...
def main():
    ...
    for ga, gb in PAIRS:
        ...
        print(
            f"  effect_A  = {f.effect_a:+.4f}  "
            f"CI [{f.ci_a[0]:+.4f}, {f.ci_a[1]:+.4f}]"
        )
        print(
            f"  effect_B  = {f.effect_b:+.4f}  "
            f"CI [{f.ci_b[0]:+.4f}, {f.ci_b[1]:+.4f}]"
        )
        print(
            f"  effect_AB = {f.effect_ab:+.4f}  "
            f"CI [{f.ci_ab[0]:+.4f}, {f.ci_ab[1]:+.4f}]"
        )
        print(f"  additive_pred (eA+eB) = {f.additive_pred:+.4f}")
        print(
            f"  interaction = {f.interaction:+.4f}  "
            f"CI [{f.ci_interaction[0]:+.4f}, {f.ci_interaction[1]:+.4f}]"
        )
        print(
            f"  bic_additive={f.bic_additive:.2f} bic_free={f.bic_free:.2f} "
            f"dBIC={f.bic_additive-f.bic_free:.2f}"
        )
        dom = "A" if abs(f.effect_a) > abs(f.effect_b) else "B"
        print(
            f"  arm magnitudes: |eA|={abs(f.effect_a):.3f} "
            f"|eB|={abs(f.effect_b):.3f}  dominant={dom}"
        )
        print(
            f"  AB - A = {f.effect_ab-f.effect_a:+.3f}   "
            f"AB - B = {f.effect_ab-f.effect_b:+.3f}"
        )

        d = axis_diag(adata, ga, gb)
        print("  --- projection-axis geometry (2000 HVG log1p space) ---")
        print(
            f"  ‖vA‖={d['norm_vA']:.3f} ‖vB‖={d['norm_vB']:.3f} "
            f"cos(vA,vB)={d['cos_AB']:+.3f}"
        )
        print(
            f"  proj(A on axis)={d['proj_A_on_axis']:+.3f} "
            f"proj(B on axis)={d['proj_B_on_axis']:+.3f} "
            f"proj(AB on axis)={d['proj_AB_on_axis']:+.3f}"
        )
        print(
            f"  combo shift decomposed: on-axis ‖={d['on_axis']:.3f}  "
            f"OFF-axis ‖={d['off_axis']:.3f}  "
            f"(off/on ratio={d['off_axis']/max(d['on_axis'],1e-9):.2f})"
        )
        print(
            f"  interaction residual (v_AB - v_A - v_B): ‖={d['resid_norm']:.3f}  "
            f"on-axis={d['resid_on_axis']:+.3f}  "
            f"off-axis ‖={d['resid_off_axis']:.3f}"
        )
